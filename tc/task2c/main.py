...
initial_at = np.array([0, 1, 2, 0, 1, 2])	# [orbit0, orbit1, orbit2]：0表示轨道上无电子，1表示轨道上一个自旋向上电子，-1表示轨道上一个自旋向下电子，2表示轨道上两个电子
initial_ph = np.array([0, 0, 0, 0, 0])	# [\Omega\uparrow, \Omega\downarrow, \omega\uparrow, \omega\downarrow, \omega_{spin}]
initial_bs = np.array([0, 0, 0, 0, 0, 0, 1, 2, 0, 1, 2])	# 初态
list_bs = []
	...
	for j in range(len(list_sink_unique)):
		if((list_sink[i] == list_sink_unique[j]).all()):
			if(i >= cnts[1] and i < cnts[2]):
				...
				cnt3 += 1
			elif(i >= cnts[4] and i < cnts[5]):
				cnt4 += 1
			elif(i >= cnts[5] and i < cnts[6]):
				cnt5 += 1
			elif(i >= cnts[6] and i < cnts[7]):
				cnt6 += 1
			cnt += 1
	if(cnt == 0):
...
cnts[5] -= cnt1 + cnt2 + cnt3 + cnt4
cnts[6] -= cnt1 + cnt2 + cnt3 + cnt4 + cnt5
cnts[7] -= cnt1 + cnt2 + cnt3 + cnt4 + cnt5 + cnt6
